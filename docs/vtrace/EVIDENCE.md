...
| EVID-066 | command output / inspection | `cargo test`, `cargo run --example lookup`, `cargo run --example ai_response_contract`, `src/lib.rs`, `docs/vtrace/INTERFACES.md`, `docs/theory/rust-lifecycle-filter-api-design.md` | Rust lifecycle report API exists, default lifecycle search matches existing search, suppressed candidates are separate from suggestions, interface summary is updated, and examples still run. | pass. | pass |
| EVID-067 | inspection | `docs/theory/review-only-catalog-data-model.md`, `docs/theory/rust-lifecycle-filter-api-design.md`, `docs/frame-catalog.md`, `docs/theory/theory-gap-audit.md`, `docs/theory/theory-roadmap.md` | Review-only catalog data model exists and keeps docs-catalog, draft, held, deprecated, rejected, and anti-pattern rows separate from accepted starter suggestions. | pass. | pass |
| EVID-068 | inspection / JSON parse | `docs/eval/review-only-catalog-fixtures.json`, `docs/eval/README.md`, `docs/theory/review-only-catalog-data-model.md`, `docs/theory/theory-gap-audit.md`, `docs/theory/theory-roadmap.md` | Review-only catalog fixture package exists, parses as JSON, and contains docs-catalog, anti-pattern, and held rows without changing Rust loading. | pass. | pass |
| EVID-069 | command output / inspection | `cargo test`, `cargo run --example lookup`, `cargo run --example ai_response_contract`, `src/lib.rs`, `docs/vtrace/INTERFACES.md`, `docs/theory/review-only-catalog-data-model.md`, `docs/theory/rust-lifecycle-filter-api-design.md` | Rust review-only catalog rows exist separately from `STARTER_CATALOG`, review helpers expose them, suppressed reports derive from review rows, default search remains accepted-only, and examples still run. | pass. | pass |
