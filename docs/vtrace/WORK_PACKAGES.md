...
| WP-067 | Broaden relation-aware fixture coverage. | Relation-aware ranking covers accepted-starter sequence and composition cases beyond hard stops. | REQ-071 / SPEC-070 / IF-068 | `docs/eval/relation-aware-ranking-fixtures.json`, `src/lib.rs`, `docs/eval/README.md`, `docs/theory/rust-relation-aware-ranking-design.md`, `docs/theory/catalog-metadata-migration-plan.md`, `docs/theory/theory-gap-audit.md`, `docs/theory/theory-roadmap.md`, `docs/vtrace/*` | WP-066 complete and relation report example exists. | New relation fixtures parse, Rust metadata/tests cover visibility-before-dependency and pace-plus-buffer cases, default search remains unchanged, examples and VTRACE pass. | L0: JSON parse; `cargo fmt --check`; `cargo test`; `cargo run --example lookup`; `cargo run --example ai_response_contract`; `cargo run --example relation_lookup`; `git diff --check` / L1: VTRACE validate / L2: deepen relation fixture coverage | evidence / trace / review / status rows | complete |
| WP-068 | Deepen relation-aware sequencing coverage. | Relation-aware ranking covers stabilization-before-reentry sequencing. | REQ-072 / SPEC-071 / IF-068 | `docs/eval/relation-aware-ranking-fixtures.json`, `src/lib.rs`, `docs/eval/README.md`, `docs/theory/rust-relation-aware-ranking-design.md`, `docs/theory/catalog-metadata-migration-plan.md`, `docs/theory/theory-gap-audit.md`, `docs/theory/theory-roadmap.md`, `docs/vtrace/*` | WP-067 complete and broader relation fixtures exist. | New relation fixture parses, Rust metadata/tests rank `shoulder-pull-off` before `merge-lane`, default search remains unchanged, examples and VTRACE pass. | L0: JSON parse; `cargo fmt --check`; `cargo test`; `cargo run --example lookup`; `cargo run --example ai_response_contract`; `cargo run --example relation_lookup`; `git diff --check` / L1: VTRACE validate / L2: continue deepening relation fixture coverage | evidence / trace / review / status rows | complete |
...
### WP-050: Define frame acquisition method

Objective: define how new frame candidates are captured, screened, scored, and
promoted before broad catalog growth or Rust index inclusion.

Parent IDs: REQ-054, SPEC-053, IF-056.
...
| V Area | IDs / Evidence | Status | Notes |
|---|---|---|---|
| Requirements | REQ-054 | closed | Acquisition method required before broad catalog growth. |
| Specification / Interface | SPEC-053, IF-056 | closed | Intake, source channels, screening, promotion, lifecycle, and index boundary defined. |
...
### WP-057: Add Rust lifecycle report API

Objective: implement additive lifecycle-aware report APIs that expose
suggestions, fallbacks, and suppressed candidates as separate channels while
preserving existing accepted-starter default search behavior.

Parent IDs: REQ-061, SPEC-060, IF-063.
...
| V Area | IDs / Evidence | Status | Notes |
|---|---|---|---|
| Requirements | REQ-061 | closed | Lifecycle reports required without expanding default search. |
| Specification / Interface | SPEC-060, IF-063 | closed | Lifecycle statuses, visibility modes, filters, result classes, display rules, suppressed reports, and report shape implemented additively. |
| Implementation | `src/lib.rs` | closed | `search_with_lifecycle` returns suggestions, fallbacks, and suppressed candidates separately. |
| Verification | EVID-066 | closed | Unit tests prove default search equivalence and suppressed report separation; interface summary updated; examples still run. |
| Validation | VAL-058 | closed | Tool callers can request lifecycle reports without changing ordinary accepted-frame lookup. |

### WP-058: Define review-only catalog data model

Objective: define a separate data model for docs-catalog, draft, held,
deprecated, rejected, and anti-pattern rows before they are loaded into
`frames-core`.

Parent IDs: REQ-062, SPEC-061, IF-064.
...
| V Area | IDs / Evidence | Status | Notes |
|---|---|---|---|
| Requirements | REQ-062 | closed | Review-only catalog model required before loading non-default rows into Rust. |
| Specification / Interface | SPEC-061, IF-064 | closed | Row families, required fields, display rules, loading boundary, conversion rules, matching rules, migration steps, and acceptance gate defined. |
...
### WP-060: Add Rust review-only catalog rows

Objective: add the first Rust review-only catalog rows and use them to back
suppressed-candidate reports without adding them to accepted starter search.

Parent IDs: REQ-064, SPEC-063, IF-066.
...
| V Area | IDs / Evidence | Status | Notes |
|---|---|---|---|
| Requirements | REQ-064 | closed | Rust review rows must stay separate from accepted starter search. |
| Specification / Interface | SPEC-063, IF-066 | closed | Review family, review entry, review helpers, row loading, and suppressed conversion boundary implemented. |
| Implementation | `src/lib.rs` | closed | `REVIEW_CATALOG` contains first review-only rows and suppression rules resolve through review entries. |
| Verification | EVID-069 | closed | Unit tests, examples, inspection, and VTRACE validation cover the review-only API boundary. |
| Validation | VAL-061 | closed | API maintainers can inspect review rows and request explanations without default recommendations. |

### WP-061: Add catalog review-mode output

Objective: expose review-only rows through explicit lifecycle review modes while
keeping them out of default suggestions.

Parent IDs: REQ-065, SPEC-064, IF-067.
...
| V Area | IDs / Evidence | Status | Notes |
|---|---|---|---|
| Requirements | REQ-065 | closed | Review-only rows must be listable for review without becoming recommendations. |
| Specification / Interface | SPEC-064, IF-067 | closed | `ReviewCandidate`, `review_only`, and explicit review-mode constructors implemented. |
| Implementation | `src/lib.rs` | closed | Catalog, anti-pattern, and docs-catalog preview modes populate `review_only` only. |
| Verification | EVID-070 | closed | Unit tests, examples, inspection, and VTRACE validation cover review-mode output. |
| Validation | VAL-062 | closed | Catalog reviewers can inspect review-only rows through lifecycle reports. |

### WP-062: Publish relation-aware ranking fixtures

Objective: publish machine-readable ranking expectations before implementing
relation-aware scoring in Rust.

Parent IDs: REQ-066, SPEC-065, IF-068.
...
| V Area | IDs / Evidence | Status | Notes |
|---|---|---|---|
| Requirements | REQ-066 | closed | Relation-aware ranking needs fixture-backed expectations before scoring changes. |
| Specification / Interface | SPEC-065, IF-068 | closed | Fixture shape covers scoring order, expected order, demotion, suppression, warnings, fallbacks, and sources. |
...
### WP-064: Add private relation metadata tables

Objective: add private relation metadata and fixture-mapped tests before any
public relation-aware scoring API.

Parent IDs: REQ-068, SPEC-067, IF-070.
...
| V Area | IDs / Evidence | Status | Notes |
|---|---|---|---|
| Requirements | REQ-068 | closed | Private relation metadata must cover fixture IDs without changing search behavior. |
| Specification / Interface | SPEC-067, IF-070 | closed | Private metadata rows and fixture-mapped tests exist while public API remains unchanged. |
...
### WP-067: Broaden relation-aware fixture coverage

Objective: add accepted-starter relation fixture coverage beyond the first
hard-stop and warning cases.

Parent IDs: REQ-071, SPEC-070, IF-068.
...
| V Area | IDs / Evidence | Status | Notes |
|---|---|---|---|
| Requirements | REQ-071 | closed | Relation-aware fixture coverage includes more accepted-starter structural cases. |
| Specification / Interface | SPEC-070, IF-068 | closed | Fixture package adds visibility/dependency and pace/buffer relation cases. |
| Implementation | `docs/eval/relation-aware-ranking-fixtures.json`, `src/lib.rs` | closed | Rust metadata and tests cover the new fixture expectations. |
| Verification | EVID-076 | closed | JSON parse, unit tests, examples, and inspection cover the expansion. |
| Validation | VAL-068 | closed | API maintainers can evaluate broader relation behavior before default-search promotion. |

### WP-068: Deepen relation-aware sequencing coverage

Objective: add a stabilization-before-reentry relation fixture and Rust test.

Parent IDs: REQ-072, SPEC-071, IF-068.
...
| V Area | IDs / Evidence | Status | Notes |
|---|---|---|---|
| Requirements | REQ-072 | closed | Relation-aware fixture coverage includes stabilization and re-entry sequencing. |
| Specification / Interface | SPEC-071, IF-068 | closed | Fixture package adds `shoulder-pull-off` before `merge-lane` sequence expectations. |
...
| Verification | EVID-077 | closed | JSON parse, unit tests, examples, and inspection cover the sequence case. |
| Validation | VAL-069 | closed | API maintainers can evaluate re-entry sequencing before default-search promotion. |

### WP-069: Add recovery-pause boundary coverage

Objective: add a recovery-pause relation fixture and Rust test that preserve
restart-condition boundaries.

Parent IDs: REQ-073, SPEC-072, IF-068.
...
| V Area | IDs / Evidence | Status | Notes |
|---|---|---|---|
| Requirements | REQ-073 | closed | Relation-aware fixture coverage includes recovery-pause restart boundaries. |
| Specification / Interface | SPEC-072, IF-068 | closed | Fixture package adds `rest-stop` before demoted `shoulder-pull-off` expectations. |
| Implementation | `docs/eval/relation-aware-ranking-fixtures.json`, `src/lib.rs` | closed | Rust metadata and tests cover the new fixture expectation. |
| Verification | EVID-078 | closed | JSON parse, unit tests, examples, and inspection cover the recovery-pause case. |
| Validation | VAL-070 | closed | API maintainers can evaluate recovery-pause boundaries before default-search promotion. |

### WP-070: Add route-adjustment boundary coverage

Objective: add a route-adjustment relation fixture and Rust test that preserve
stable-destination boundaries.

Parent IDs: REQ-074, SPEC-073, IF-068.
...
| V Area | IDs / Evidence | Status | Notes |
|---|---|---|---|
| Requirements | REQ-074 | closed | Relation-aware fixture coverage includes route-adjustment destination boundaries. |
| Specification / Interface | SPEC-073, IF-068 | closed | Fixture package adds `detour` before demoted `shoulder-pull-off` expectations. |
| Implementation | `docs/eval/relation-aware-ranking-fixtures.json`, `src/lib.rs` | closed | Rust metadata and tests cover the new fixture expectation. |
| Verification | EVID-079 | closed | JSON parse, unit tests, examples, and inspection cover the route-adjustment case. |
| Validation | VAL-071 | closed | API maintainers can evaluate route-adjustment boundaries before default-search promotion. |

### WP-071: Add reserve-tracking boundary coverage

Objective: add a reserve-tracking relation fixture and Rust test that preserve
scarce-resource boundaries.

Parent IDs: REQ-075, SPEC-074, IF-068.
...
| V Area | IDs / Evidence | Status | Notes |
|---|---|---|---|
| Requirements | REQ-075 | closed | Relation-aware fixture coverage includes reserve-tracking scarce-resource boundaries. |
| Specification / Interface | SPEC-074, IF-068 | closed | Fixture package adds `fuel-gauge` before demoted `red-yellow-green` expectations. |
| Implementation | `docs/eval/relation-aware-ranking-fixtures.json`, `src/lib.rs` | closed | Rust metadata and tests cover the new fixture expectation. |
